...
class GradModL2P(nn.Module):
    def __init__(self,
        ...
        self.register_buffer('mean_prompts', torch.zeros(selection_size, self.backbone.num_features))
        self.register_buffer('prompts',      torch.zeros(selection_size, self.backbone.num_features))
    ...
    def forward(self, inputs : torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        if self.training : 
            ...
            m = p.mean(1).unsqueeze(1)
            p = p - m
            p = torch.concat((m, p), dim=1)
            p = p + self.pos_embed[:,0].expand(self.selection_size * self.prompt_len + 1, -1)
        ...
        if self.training:
            x = x + self.mask
            self.prompts = self.prompts + self.mask
            self.mean_prompts = self.mean_prompts + self.mask
        return x
    
    def loss_fn(self, output, target):
        B, C = output.size()
        L_ce = F.cross_entropy(self.mean_prompts, target)
        L_dd = F.cross_entropy(self.prompts.transpose(-1,-2), target.unsqueeze(1).expand(B, self.selection_size * self.prompt_len)).min()
        return self.gamma * L_ce + (1 - self.gamma) * L_dd - self.lambd * self.simmilarity
