...
class ContrastiveL2P(nn.Module):
    def __init__(self,
        ...
        self.register_buffer('positive', torch.zeros(1))
        self.register_buffer('negative', torch.zeros(1))
        self.register_buffer('mask', torch.zeros(class_num))

    def forward_feature(self, inputs) -> torch.Tensor:
    ...
    def forward(self,
        ...
        if pos is not None:
            p = self.forward_feature(pos)
        if neg is not None:
            n = self.forward_feature(neg)
            
        x = self.forward_feature(inputs)

        # B 1 D
        # B D 1
        # B D @ D B
        if pos is not None:
            self.positive = (x.unsqueeze(1) @ p.unsqueeze(-1)).squeeze()
        if neg is not None:
            self.negative = (x.unsqueeze(1) @ n.unsqueeze(-1)).squeeze()
    ...
    def loss_fn(self, output, target):
        B, C = output.size() 
        return F.cross_entropy(output, target) - self.lambd * self.simmilarity - 2 * ((self.positive/0.5).exp() / ((self.negative/0.5).exp() + (self.positive/0.5).exp())).log().sum()
