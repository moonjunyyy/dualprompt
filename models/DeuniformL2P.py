...
class DeuniformL2P(nn.Module):
    def __init__(self,
        ...
        self.register_buffer('deuniform',   torch.zeros(1))
    ...
    def forward(self, inputs : torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        mean = x.mean()
        self.deuniform =  (x * x).mean() / (mean * mean) - 1
    ...
    def loss_fn(self, output, target):
        B, C = output.size() 
        return F.cross_entropy(output, target) - self.lambd * self.simmilarity - self.xi * self.deuniform
